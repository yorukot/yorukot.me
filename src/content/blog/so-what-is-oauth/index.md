...
The reason is that PKCE mainly solves a problem for public clients, and public clients cannot safely store a `client_secret`.
...
The `code_verifier` is a random string generated by the client.
...
The method can be `S256` or `plain`. `S256` stands for SHA-256, and it is the recommended method. If the client supports `S256`, it should use it.
...
If it matches, the server can issue the `access_token` and, optionally, a `refresh_token`.

## Do I Really Need PKCE?

PKCE is part of current best practice, as described in [RFC 9700](https://datatracker.ietf.org/doc/html/rfc9700). In practice, you should treat it as the default. For public clients, it is a must. For confidential clients, even though they are usually safer because they can store credentials securely, PKCE still adds another layer of protection and is worth using.
# Summary
